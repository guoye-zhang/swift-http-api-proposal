...
struct UnsupportedPlatformHTTPServer: HTTPServer {
    ...
    typealias RequestConcludingReader = ConcludingReader
    typealias ResponseConcludingWriter = ConcludingWriter

    func serve(
        handler: some HTTPServerRequestHandler<ConcludingReader, ConcludingWriter>
    ) async throws {
        fatalError("Unspported platform")
    }
}
